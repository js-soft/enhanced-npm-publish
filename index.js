...
async function existsPackageInRegistry(pkg) {
  return await pacote.packument(pkg.name).then(
    ...
    () => {
      console.warn(
        `Unable to determine published version, assuming ${pkg.name} unpublished.`,
      );
      return false;
    },
  );
...
async function run() {
  ...
  if (process.argv.includes("--only-prerelease")) {
    ...
    if (!semverMatches || !semverMatches[1]) {
      console.error(
        `The version '${pkg.version}' of package '${pkg.name}' is not a prerelease version, aborting.`,
      );
      process.exit(0);
    }
  }
  ...
  if (process.argv.includes("--if-possible") && isPublished) {
    console.log(
      `${pkg.name} with version ${pkg.version} is already published.`,
    );
    process.exit(0);
  }
  ...
  if (
    process.argv.includes("--use-preid-as-tag") &&
    semverMatches &&
    semverMatches[1]
  ) {
